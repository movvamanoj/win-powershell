# Step 0: Check if any disk has already been allocated a drive letter
...
if ($allocatedDisks.Count -gt 0) {
    ...
    # Step 2: Formatting - Format the volumes with NTFS file system for unallocated disks
    foreach ($disk in $unallocatedDisks) {
        $diskNumber = $disk.Number
    ...
    # Continue with the rest of the script (additional steps if needed)...
}
